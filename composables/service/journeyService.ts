...
import { ACTIVITY_TYPES_SELECT_STRING, type ActivityApiType } from '~/utils/const/activityTypes'
import { ref } from 'vue'
...
export async function fetchBaseJourneyFromId(journeyId: string): Promise<BaseJourney | undefined> {
...
export async function fetchBaseJourneyAnswersFromId(journeyId: string): Promise<Answer[] | undefined> {
  const db = useFirestore()
  if (!db) throw new Error('Firestore is not initialized')
...
function buildActivityQueryParams(answers: Answer[], baseJourney: BaseJourney) {
  ...
  const isSomeoneCheap = answers.map(answer => answer.activityPriceRange[0]).includes(0)
  const isFree = isSomeoneCheap ? "price_type='gratuit'" : ''

  const allTags = new Set(answers.flatMap(answer => answer.activities))

  const tagFilter = baseJourney.type === ActivityType.random ? '' :
    Array.from(allTags)
  ...
  const whereQuery = [needPmr, isFree, tagFilter, publicFilter].filter(Boolean).join(' and ')
...
function journeyTypePublicFilterValues(journeyType: ActivityType): string[] {
  switch (journeyType) {
    case ActivityType.family: return ['Tout public', 'Public tout-petits', 'A partir de 3 ans', 'Public jeune', 'Public tout-petits et enfants']
    case ActivityType.friends: return ['A partir de 15 ans', 'Public jeunes et adultes']
    case ActivityType.romantic: return ['Public adulte', 'A partir de 15 ans']
    case ActivityType.random: return []
  }
}

export async function findActivityFromAnswers(answers: Answer[], baseJourney: BaseJourney): Promise<ActivityApiType[]> { 
  const queryParams = buildActivityQueryParams(answers, baseJourney);
  const response = await fetch(`https://opendata.paris.fr/api/explore/v2.1/catalog/datasets/que-faire-a-paris-/records?${queryParams}`);
  
  if (!response.ok) throw new Error("Aucune activité trouvée");
  
  const data = await response.json();
  
  if (!data.results || data.results.length === 0) throw new Error("Aucune activité trouvée");
