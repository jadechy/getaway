...
const activityTypesList = Object.entries(ACTIVITY_TYPES_LIST).map(([id, label]) => ({
  id,
  label
}));
...
const updatePrice = (e: Event, index: 0 | 1) => {
  ...
  const newPrice: [number, number] = [...props.formAnswers.answerPriceRange] as [number, number];
