...
// import PrimaryButton from '@/components/general/buttons/PrimaryButton.vue'
...
import { completeJourney, fetchBaseJourneyAnswersFromId, fetchBaseJourneyFromId, findActivityFromAnswers } from '~/composables/service/journeyService'
import { recapJourneyInformation } from '~/utils/journey/recapJourney'
import { searchRestaurantsByTypes } from '~/composables/useJourney'
import {formatActivityFromAPIToStep, formatRestaurantFromBDDToStep} from '@/utils/journey/formatStepJourney'
...
onBeforeMount(async () => {
  ...
  // if (!journey?.status || journey.status !== JourneyStatus.readyForGeneration) {
  ...
  if (answers && journey) {
    ...
    const allAnswersType = Array.from(new Set(answers.flatMap((a: any) => a.restaurant)))
    const priceList = answers.map((a: any) => a.restoPriceRange[1])
    const minPrice = Math.min(...priceList)
    restaurantsList.value = await searchRestaurantsByTypes(allAnswersType, journey.type, minPrice)
  }
})
...
function shuffleRest() {
  restaurant.value = (restaurant.value + 1) % (restaurantsList.value?.length || 1)
}
  ...
  <div class="px-4 py-6 space-y-6">
    ...
    <div class="space-y-6">
      <StepPreviewWithReload :reload="shuffleAct1">
        <StepPreview :step="formatActivityFromAPIToStep(activityList?.[activity1], true)" />
      </StepPreviewWithReload>

      <StepPreviewWithReload v-if="restaurantsList?.length" :reload="shuffleRest">
        <StepPreview :step="formatRestaurantFromBDDToStep(restaurantsList[restaurant])" />
      </StepPreviewWithReload>

      <p v-else>Aucun restaurant trouvé...</p>

      <StepPreviewWithReload v-if="baseJourney?.isFullDay" :reload="shuffleAct2">
        <StepPreview :step="formatActivityFromAPIToStep(activityList?.[activity2], false)" />
      </StepPreviewWithReload>
    </div>

    <PrimaryButton label="Régénérer" @click="handleRegenerate" backgroundColor="#333" />

    <PrimaryButton
      :label="baseJourney?.isFullDay ? 'Enregistrer la journée' : 'Enregistrer la demi-journée'"
      @click="handleSave"
    />
  </div>
</template>
